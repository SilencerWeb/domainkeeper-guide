...
Для начала нужно определиться с версий: *build* или *inline*? Чтобы понять какая именно вам нужна, вам нужно задать один вопрос: Вы собираете изменять стили(CSS) и/или скрипты(JS)? Если нет, то смело выбирайте *inline*, если да, то выбирайте *build*. К тому же, *inline* более оптимизирована чем build, поэтому мы рекомендуем использовать именно ее.
